...
If($osVersion -eq 10){
    ...
    Write-Log "Windows 10 OS detected"
    $printers = @()
    ...
    Foreach($printer in $printers){
        ...
        If($printer.Type -eq "Connection"){
            ...
            Write-Log "Found network printer mapped: $printerDeviceName. The full name of the printer is: $fullPrinterName" -Severity 2                    
            #Write-Log "The printer device name is $printerDeviceName"
            ...
            If($printerDeviceName -eq $myPrinterUpdatedVariable){
                Write-Log "$printerDeviceName was found on the PrinterList with migrated printers. Checking it's server to see if it's mapped from the old one."
                [string]$currentPrintServer = $printer.ComputerName
                Write-Log "The Print Server name for this mapped printer is: $currentPrintServer" -Severity 2
        
                $myServ = $csv | Where-Object {$_.OldServer -eq "$currentPrintServer"}
                #Write-Log "MyServ is: $myServ"
        
                $oldServer = $myServ.oldServer

                Write-Log "Checking if $currentPrintServer is found in the `"OldServer`" column in the CSV file..."
                ...
                If($currentPrintServer -eq $oldServer){
                    ...
                    Write-Log "$currentPrintServer was found in the `"OldServer`" column in the CSV file! It's coresponding new value will be $newServer. Performing update..." -Severity 2
                    Write-Log "Deleting $printerDeviceName" -Severity 3
                    ...
                    Write-Log "The new full printer name is going to be: $newFullPrinterName"
                    Write-Log "The new print server for the printer is going to be: $newPrinterServer"
                    Write-Log "Remapping printer..."
                    ([wmiclass]"Win32_Printer").AddPrinterConnection($newFullPrinterName)
                }else{
                    Write-Log "$currentPrintServer was NOT found in the `"OldServer`" column in the CSV file! Nothing to do..."
                }
                #############################################
                ## ^ end IF#3 - print server name check
                #############################################
            }else{
                Write-Log "$printerDeviceName was not found in the PrinterList with the migrated printers. This printer won't be migrated. Nothing to do..."
            }
            ####################################
            ...
            Write-Log "$localPrinter is local. Not touching this printer"
        }
        ################################################################
    ...
    Write-Log "Windows 7 OS detected"
    $printers = @()
    ...
    Foreach($printer in $printers){
        ...
        If($printer.Network -eq $true){
            ...
            If($printerDeviceName -eq $myPrinterUpdatedVariable){
                ...
                If($currentPrintServer -eq $oldServer){
                    $newServer = $myServ.NewServer
                    Write-Log "$currentPrintServer found in the `"OldServer`" column in the CSV file! It's coresponding new value will be $newServer. Performing update..." -Severity 2
                    Write-Log "Deleting $printerDeviceName" -Severity 3
                    Get-WmiObject Win32_Printer | where{$_.ShareName -eq $printerDeviceName} | foreach{$_.delete() }
            
                    $newFullPrinterName = $printer.Name -replace($oldServer,$newServer)
                    $newPrinterServer = $printer.ServerName -replace("$oldServer","$newServer")
                    Write-Log "The new printer name is going to be: $newFullPrinterName"
                    Write-Log "The new print server for the printer is going to be: $newPrinterServer"
                    Write-Log "Remapping printer..."
                    ([wmiclass]"Win32_Printer").AddPrinterConnection($newFullPrinterName)
                }else{
